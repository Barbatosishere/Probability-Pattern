...
需要 **Java 21** 来运行 Gradle（Java 25 在 Gradle/Groovy 配置阶段会失败）。

### 方式一：针对本地源码构建（推荐调试）

项目使用 Gradle 复合构建（composite build），`settings.gradle` 中的 `includeBuild('..')` 会将 AE2 依赖替换为上级目录中已检出的 AE2 源码：
...
移除 `settings.gradle` 中的 `includeBuild('..')` 语句，然后在 `gradle.properties` 中设置 `ae2_version` 为你需要的版本号即可。
...
├── terminal/
│   ├── ProbabilityPatternTerminalBlock.java       # 方块定义
│   ├── ProbabilityPatternTerminalBlockEntity.java # 方块实体

...
public class SPCraftingTreeNode {
    ...
    // parent node.
    private final SPCraftingTreeProcess parent;
     ...
     */
    private ArrayList<SPCraftingTreeProcess> nodes = null;
    private final boolean canEmit;

    public SPCraftingTreeNode(ICraftingService cc, SPCraftingCalculation job, AEKey what, long amount, SPCraftingTreeProcess par, int slot) {
        this.parent = par;
            ...
            if (gridNode != null) {
                ...
                for (var details : craftingService.getCraftingFor(this.what)) {
                    if (this.parent == null || this.parent.notRecursive(details)) {
                        this.nodes.add(new SPCraftingTreeProcess(craftingService, job, details, this));
                    }
                }
            }
        }
    ...
    void request(CraftingSimulationState inv, long requestedAmount,
        ...
        if (this.nodes.size() == 1) {
            ...
            final SPCraftingTreeProcess pro = this.nodes.get(0);
            ...
            for (SPCraftingTreeProcess pro : this.nodes) {
                try {
                    while (pro.possible && totalRequestedItems > 0) {
